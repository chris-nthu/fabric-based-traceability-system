 ...
 */
 ...
 type Product2 struct {
	GPS_Location	float64	`json:"location"`
 ...
 func (s *SmartContract) testSubmit(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	for i < len(products) {
		...
		location_float := randFloats(0, 180)
		temperature_float := randFloats(0, 35)
		humidity_float := randFloats(0, 100)

		products[i] = Product2{GPS_Location: location_float, Temperature: temperature_float, Humidity: humidity_float}
